...
import { useGLTF, useTexture, Environment, Lightformer, Preload, AdaptiveDpr, AdaptiveEvents } from '@react-three/drei';
import { BallCollider, CuboidCollider, Physics, RigidBody, useRopeJoint, useSphericalJoint } from '@react-three/rapier';
import { MeshLineGeometry, MeshLineMaterial } from 'meshline';
import * as THREE from 'three';

// Load assets via public/ to avoid webpack loaders for binary imports
...
export default function Lanyard({ position = [0, 0, 30], gravity = [0, -40, 0], fov = 20, transparent = true, fullScreen = true, scaleMultiplier = 2.25 }: LanyardProps) {
  ...
  return (
    <div className={fullScreen ? "lanyard-overlay" : "lanyard-embed"}>
      <Canvas
        ...
        shadows={false}
        flat
        gl={{ alpha: transparent, antialias: true, powerPreference: 'high-performance', stencil: false, preserveDrawingBuffer: false }}
        onCreated={({ gl }) => {
          gl.setClearColor(new THREE.Color(0x000000), transparent ? 0 : 1);
          (gl as any).setClearAlpha?.(transparent ? 0 : 1);
          const el = (gl as unknown as { domElement: HTMLCanvasElement }).domElement;
          const onLost = (e: Event) => {
            e.preventDefault();
            // Remount canvas to restore context smoothly
...
function Band({ maxSpeed = 50, minSpeed = 0, scaleMultiplier = 2.25 }: { maxSpeed?: number; minSpeed?: number; scaleMultiplier?: number }) {
  ...
  useFrame((state, delta) => {
    ...
    const setPoint = (point: THREE.Vector3, t: { x: number; y: number; z: number }) => {
    ...
    <>
      <group position={[0, anchorY, 0]}>
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...(segmentProps as any)} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          ...
          >
            {(() => {
              const cardMesh = (nodes as any)?.card ?? (scene as any)?.children?.find((c: any) => c?.name?.toLowerCase?.() === 'card' || c?.isMesh);
              const clipMesh = (nodes as any)?.clip ?? (scene as any)?.getObjectByName?.('clip');
              const clampMesh = (nodes as any)?.clamp ?? (scene as any)?.getObjectByName?.('clamp');
              const baseMap = (materials as any)?.base?.map ?? (cardMesh?.material?.map ?? null);
              return (
                <>
                  {cardMesh && (
                  ...
                  {clipMesh && <mesh geometry={clipMesh.geometry} material={(materials as any)?.metal ?? clipMesh.material} material-roughness={0.3} />}
                  {clampMesh && <mesh geometry={clampMesh.geometry} material={(materials as any)?.metal ?? clampMesh.material} />}
                </>
              );
            })()}
          </group>
